...
class UNet(nn.Module):
    ...
    def conv_block(self, in_channels, out_channels, k=2):
        """
        Creates a block of TWO consecutive Convolution -> Norm -> ReLU sequences.
        Uses padding='same' to preserve spatial dimensions during convolutions.
        """
        if self.gn_enable:
            norm1 = nn.GroupNorm(self.num_groups, out_channels)
            norm2 = nn.GroupNorm(self.num_groups, out_channels)
        else:
    ...
    def forward(self, x):
        ...
        # Final 1x1 Conv directly from decoder output d1
        out = self.final_output(d1)
