...
class DownscalingViT(nn.Module):
    ...
    def forward(self, x):
        ...
        # GPU ADAPTATION: Dynamically detect the device of the current batch (patches.device).
        # This guarantees that the dynamically generated positional embedding is placed 
        # on the same GPU/CPU as the input data, preventing runtime device mismatches.
        device = patches.device  

        if (self.positional_embedding is None or
                self.positional_embedding.shape[1] != num_patches):
            self.positional_embedding = nn.Parameter(
                torch.randn(1, num_patches, patches.shape[2], device=device)
            )
