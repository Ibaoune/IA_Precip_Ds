...
#SBATCH --job-name=cnn_exp3_north_gpu
#SBATCH --output=cnn_exp3_north_gpu%j.log
#SBATCH --error=cnn_exp3_north_gpu%j.log
...
echo "Model: cnn_exp3 (NORTH) | Train: $train | Validation: $validation"
echo "======================================"

nvidia-smi

gpu_log="gpu_usage_cnn_exp3_north_${SLURM_JOB_ID}.log"
nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,memory.used \
           --format=csv,nounits,noheader \
           --loop=60 > "$gpu_log" &

CONFIG="/srv/data/mohammad.elaabaribao/work/papers/downscaling/main/configs/cnn/regional/cnn_exp3_north.yaml"
