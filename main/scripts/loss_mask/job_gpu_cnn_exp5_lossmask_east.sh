...
#SBATCH --job-name=cnn_exp5_lm_east
#SBATCH --output=cnn_exp5_lm_east_%j.log
#SBATCH --error=cnn_exp5_lm_east_%j.log
...
echo "Model: cnn_exp5 (Loss Mask: EAST) | Train: $train | Validation: $validation"
echo "======================================"

nvidia-smi

gpu_log="gpu_usage_cnn_exp5_lm_east_${SLURM_JOB_ID}.log"
nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,memory.used \
 --format=csv,nounits,noheader \
 --loop=60 > "$gpu_log" &

CONFIG="/srv/data/mohammad.elaabaribao/work/papers/downscaling/main/configs/cnn/loss_mask/cnn_exp5_lossmask_east.yaml"
