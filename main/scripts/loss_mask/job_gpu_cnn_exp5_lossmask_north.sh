#!/bin/bash
#SBATCH --job-name=cnn_exp5_lm_north
#SBATCH --output=cnn_exp5_lm_north_%j.log
#SBATCH --error=cnn_exp5_lm_north_%j.log
...
echo "Model: cnn_exp5 (Loss Mask: NORTH) | Train: $train | Validation: $validation"
echo "======================================"

nvidia-smi

gpu_log="gpu_usage_cnn_exp5_lm_north_${SLURM_JOB_ID}.log"
nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,memory.used \
           --format=csv,nounits,noheader \
           --loop=60 > "$gpu_log" &

CONFIG="/srv/data/mohammad.elaabaribao/work/papers/downscaling/main/configs/cnn/loss_mask/cnn_exp5_lossmask_north.yaml"
