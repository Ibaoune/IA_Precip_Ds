...
#SBATCH --job-name=vit_lm_north
#SBATCH --output=vit_lm_north_%j.log
#SBATCH --error=vit_lm_north_%j.log
...
echo "Model: vit (Loss Mask: NORTH) | Train: $train | Validation: $validation"
echo "======================================"

nvidia-smi

gpu_log="gpu_usage_vit_lm_north_${SLURM_JOB_ID}.log"
nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,memory.used \
           --format=csv,nounits,noheader \
           --loop=60 > "$gpu_log" &

CONFIG="/srv/data/mohammad.elaabaribao/work/papers/downscaling/main/configs/vit/loss_mask/vit_lossmask_north.yaml"
