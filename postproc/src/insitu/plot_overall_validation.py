...
Description: Creates a comprehensive multi-panel validation summary combining spatial station maps and aggregated metric heatmaps.
"""
...
# --- Configuration ---
STATIONS = []  # Will be populated from config in main()
...
OUT_DIR = ""  # Will be populated from config in main()
...
def plot_annual_cycles(obs_df, stations_meta, models_data):
    ...
    for i, station in enumerate(STATIONS):
        ...
        for name in MODELS:
            ...
            mod_da = utils_insitu.extract_nearest_gridpoint(info["ds"], lat, lon, info["var_name"])
...
def main():
    config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../insitu/config_evaluation.yaml"))
    config = utils.load_config(config_path)
    
    global STATIONS, OUT_DIR
    STATIONS = config.get("parameters", {}).get("stations", [])
    OUT_DIR = config.get("output", {}).get("dir", "")
    os.makedirs(OUT_DIR, exist_ok=True)
    
    # 1. Load Station Observations
    obs_df = utils_insitu.load_insitu_observations(start_date=START_DATE, end_date=END_DATE)
