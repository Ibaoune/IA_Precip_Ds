...
def main():
    ...
    MODEL_PATHS_BASE = {
        d['name'].upper(): os.path.join(DATA_RESULTS_DIR, f"{d['name']}_{suffix}_strategy_{STRATEGY}_corr_per_year_PERIOD.nc")
    ...
    for period in plot_periods:
        ...
        utils.plot_spatial_maps(data_spatial, METRIC, period=period, save_path=spatial_path, unit="%")
        ...
        # Calculate error: Model - Reference
        ref_name = next(iter(data_raw)).upper() # Assumes reference is first
        ref_map = data_spatial[ref_name]
        error_dict = {name: ds - ref_map for name, ds in data_spatial.items() if name != ref_name}
        
        if error_dict:
            utils.plot_spatial_maps(error_dict, "r01_error", period=period, save_path=error_path, unit="%", title=f"{period} R01 Spatial Error")
