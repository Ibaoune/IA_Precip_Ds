...
def compute_r95_amount(dataset, ref=None, **kwargs):
 """
 Compute R95 Amount: Mean intensity of days >= P95.
 P95 is calculated from the reference (if provided) or the dataset itself,
 ...
 p95 = wet_days_ref.quantile(0.95, dim='time', skipna=True)
 
 def _apply_p95(da, thresh):
...
def main():
 ...
 print(f"[DEBUG] Processing reference data for R95: {reference_info['name']}...")
 ...
 metric_func=compute_r95_amount,
 model_name=dat["name"],
 ...
 # Reference R95 for this period
 if period == "Annual":
 ref_period = var_ref
 else:
 ref_period = var_ref.sel(time=var_ref.time.dt.season == period)
 
 ref_map = compute_r95_amount(dataset=ref_period, return_by_year=False)
 ...
 # Plot 1: R95 Comparison Map
 utils.plot_spatial_maps(spatial_data_dict, "r95", period=period, 
 save_path=os.path.join(period_fig_dir, f"spatial_r95_comparison_{period}.png"),
 title=f"{period} Spatial R95 Comparison ({params['predictand'].upper()})", 
 unit="mm", nrows=nrows)
 
 # Plot 2: R95 Error Map
 if spatial_error_dict:
 utils.plot_spatial_maps(spatial_error_dict, "r95_error", period=period, 
 save_path=os.path.join(period_fig_dir, f"spatial_r95_error_{period}.png"),
 title=f"{period} Spatial R95 Error ({params['predictand'].upper()})", 
 unit="mm", nrows=nrows)

 if "Annual" in plot_periods:
 # Distribution / Evolution for R95? (Not usually implemented in the script but let's see)
 pass
