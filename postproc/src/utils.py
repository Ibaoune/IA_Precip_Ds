...
def get_shapefile(region=None):
    ...
    mapping = {
        "allmorr": os.path.join(PROJECT_ROOT, "shape_files/morocco_unified_fixed_v2.shp"),
        "north": os.path.join(PROJECT_ROOT, "shape_files/north.shp"),
        "north_east": os.path.join(PROJECT_ROOT, "shape_files/north_east.shp"),
        "south": os.path.join(PROJECT_ROOT, "shape_files/south.shp"),
        "east": os.path.join(PROJECT_ROOT, "shape_files/east.shp"),
    }
...
def get_custom_colormap(metric_name, map_type='spatial'):
    ...
    if os.path.exists(config_path):
        try:
            ...
            print(f"Warning loading colorbar config: {e}")
    return None
...
def plot_spatial_maps(data_dict, metric_name, period="Annual", shapefile=None, save_path=None, title=None, levels=None, unit="mm/day", nrows=1, region=None):
    ...
    for i, (model_name, data) in enumerate(data_dict.items()):
        ...
        # Ensure lat is sorted descending to match origin="upper" in imshow
...
def plot_spatial_bias(data_dict, label='Bias (mm/day)', bounds=None, shapefile=None, save_path=None, fig_title=None, region=None):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("custom_bias", BIAS_RDBU_WHITE, N=len(bounds) + 1)
