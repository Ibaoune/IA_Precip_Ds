...
fig_dir = os.path.join(root_path, "results", "GLM_CNN_Unet_Vit_retained", "2006-01-01_2020-12-31", "allmorr", "test", "bias", "figures", "all")

mean_out_path = os.path.join(fig_dir, "mean_precipitation_Annual.png")
bias_out_path = os.path.join(fig_dir, "spatial_bias_error_Annual.png")
...
# ==========================================
# 1. Plot Individual Mean Precipitation
# ==========================================
fig_mean = plt.figure(figsize=(24, 7), dpi=300)
gs_mean = gridspec.GridSpec(1, 5, figure=fig_mean, wspace=0.0)

precip_levels = [0, 0.1, 0.2, 0.3, 0.5, 1, 2, 3, 4, 5, 6, 8, 10]
n_bins_mean = len(precip_levels) - 1
...
for i, (name, data) in enumerate(mean_dict.items()):
    ax = fig_mean.add_subplot(gs_mean[0, i], projection=ccrs.PlateCarree())
    axes_mean.append(ax)
...
# Colorbar for Mean
cbar_ax1 = fig_mean.add_axes([0.3, -0.05, 0.4, 0.03])
cbar1 = fig_mean.colorbar(im_mean, cax=cbar_ax1, orientation='horizontal', ticks=precip_levels, extend='max')
cbar1.set_label("Mean Precipitation (mm/day)", fontsize=14, fontweight='bold')
cbar1.ax.tick_params(labelsize=11)

fig_mean.savefig(mean_out_path, bbox_inches='tight', dpi=300)
print(f"Saved {mean_out_path}")


# ==========================================
# 2. Plot Individual Spatial Bias Error
# ==========================================
fig_bias = plt.figure(figsize=(24, 7), dpi=300)
# We want 4 panels centered. We can use a gridspec of 1 row, 4 cols.
gs_bias = gridspec.GridSpec(1, 4, figure=fig_bias, wspace=0.0)

bias_levels = [-2, -1, -0.5, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.5, 1, 2]
n_bins_bias = len(bias_levels) - 1
...
cbar2 = fig_bias.colorbar(im_bias, cax=cbar_ax2, orientation='horizontal', ticks=bias_levels, extend='both')
cbar2.set_label("Spatial Bias Error (mm/day)", fontsize=14, fontweight='bold')
cbar2.ax.tick_params(labelsize=11)

fig_bias.savefig(bias_out_path, bbox_inches='tight', dpi=300)
print(f"Saved {bias_out_path}")
